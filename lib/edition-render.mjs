...
export function renderEditionHtml(markdown) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (HR_RE.test(line)) { html += '<hr class="ed-hr">'; continue; }

    // Tableau GFM (ex. le bloc Sources) : une ligne avec | suivie d'une ligne séparatrice.
